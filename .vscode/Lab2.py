...
def get_user_input():
    ...
    all_temps = [float(num) for num in (user_input.split(","))]#this is to convert to string into float 
    #[float(variable_name) for variable_name in name of  the string e.g. all_temps ]
    return all_temps
...
def calc_average(all_temps):
    ...
    length = len(all_temps)# len shows the number of elements in the string
